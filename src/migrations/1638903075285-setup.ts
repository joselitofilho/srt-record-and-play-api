...
export class setup1638903075285 implements MigrationInterface {
    name = 'setup1638903075285'

    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`CREATE TABLE \`scenario_status\` (\`id\` int NOT NULL AUTO_INCREMENT, \`status\` varchar(255) NOT NULL, \`scenarioId\` int NULL, UNIQUE INDEX \`REL_46bd89d23a8d291bfbd3deba98\` (\`scenarioId\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`scenario\` (\`id\` int NOT NULL AUTO_INCREMENT, \`title\` varchar(255) NOT NULL, \`context\` varchar(255) NOT NULL, \`isTemplate\` tinyint NOT NULL DEFAULT 0, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
