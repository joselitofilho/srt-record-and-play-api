  ...
  implements MigrationInterface
{
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE \`action\` ADD CONSTRAINT \`FK_af62aad8709959780767509aa75\` FOREIGN KEY (\`scenarioId\`) REFERENCES \`scenario\`(\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`,
    );
  }
